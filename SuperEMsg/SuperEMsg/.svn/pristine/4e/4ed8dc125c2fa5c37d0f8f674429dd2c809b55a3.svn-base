...
public class ImageResizer extends ImageWorker {
	...
	private boolean isNeedCropHeight;
	...
	private Bitmap processBitmap(int resId) {
		...
		if (!isNeedCropHeight) {
			...
			final Bitmap b = decodeSampledBitmapFromResource(mContext.getResources(), resId,
					mImageWidth, mImageHeight);
			int w = b.getWidth();
			int h = b.getHeight();
			return Bitmap.createBitmap(b, 0, 0, w, h - mCropHeight, null, false);
	...
	public static Bitmap decodeSampledBitmapFromResource(Resources res, int resId, int reqWidth,
		...
		final Bitmap b = BitmapFactory.decodeResource(res, resId, options);
		return b;
	}
	...
	public static synchronized Bitmap decodeSampledBitmapFromFile(String filename, int reqWidth,
		...
		// Decode bitmap with inSampleSize set
		options.inJustDecodeBounds = false;
		final Bitmap b = BitmapFactory.decodeFile(filename, options);
		return b;
	}
	...
	public static int calculateInSampleSize(BitmapFactory.Options options, int reqWidth,
			int reqHeight) {
		//如果ImageWorker.INSAMPLESIZE已经设置值，则返回；如果为1，则继续计算
		if (getInsampleSize() != 1) {
			return getInsampleSize();
		}
