...
public class SmsListAdapter extends AbsCommonAdapter<SmsContentDetail> {

	public SmsListAdapter(Context ctx) {
		super(ctx, R.layout.item_sms_list);
	}

	public SmsListAdapter(Context ctx, int layout) {
		super(ctx, layout);
	}

	@Override
	public void getView(int position, View convertView,
			final SmsContentDetail data) {
					...
					public void onClick(View v) {
						...
						if (data.isCollected()) {
							DBUtils.getInst().insert(
									DBHelper.TABLE_SMS_COLLECTION, null, data);
						} else {
							DBUtils.getInst().remove(
									DBHelper.TABLE_SMS_COLLECTION, data);
						}
