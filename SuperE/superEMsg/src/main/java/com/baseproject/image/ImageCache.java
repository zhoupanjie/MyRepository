...
public class ImageCache {
	...
	public static ImageCache findOrCreateCache(final FragmentActivity activity,
		...
		ImageCacheParams mImageCacheParams = new ImageCacheParams(uniqueName);
		// Use 1/8th of the available memory for this memory cache.
		int cacheSize = 1024 * 1024 * Utils.getMemoryClass(activity) / 8;
		if (cacheSize > 0) {
			mImageCacheParams.memCacheSize = cacheSize;
		}
		return findOrCreateCache(activity, mImageCacheParams);
	}
	...
	public void addBitmapToCache(String data, Bitmap bitmap) {
		...
		String fileName = Utils.urlToFileName(data);
		// Add to memory cache
		if (mMemoryCache != null && mMemoryCache.get(fileName) == null) {
			mMemoryCache.put(fileName, bitmap);
		}

		// Add to disk cache
		if (mDiskCache != null && !mDiskCache.containsKey(fileName)) {
	...
	public Bitmap getBitmapFromMemCache(String data) {
		if (mMemoryCache != null) {
			String fileName = Utils.urlToFileName(data);
			final Bitmap memBitmap = mMemoryCache.get(fileName);
			// 增加对图片回收状态的判断
	...
	public Bitmap getBitmapFromDiskCache(String data) {
		if (mDiskCache != null) {
			String fileName = Utils.urlToFileName(data);
			return mDiskCache.get(fileName);
		}
		return null;
	}
