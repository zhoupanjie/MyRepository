...
public class FriendsFreshInfoNativeAdapter extends
		BaseAdapter<FriendsFreshsMoodInfoDataComment> {
	...
	public View getView(final int position, View view, ViewGroup arg2) {
		final FriendsFreshsMoodInfoDataComment item = getItem(position);
		ViewHolder holder;
		if (view == null) {
		...
		ImageLoader.getInstance().displayImage(item.getSenduser().getLogo(),
				holder.userLogo,
		...
		if (!TextUtils.isEmpty(item.getSenduser().getUserid())) {
			if (item.getSenduser().getUserid().equals(Constants.getUid())) {
		...
		if (!TextUtils.isEmpty(item.getSenduser().getUserid())) {
			...
			if (userId.equals(item.getSenduser().getUserid())) {
				span.append("我");
			} else if (!TextUtils
					.isEmpty(SelectNameUtil.getName("", item.getSenduser()
							.getNickname(), item.getSenduser().getName()))) {
				span.append(SelectNameUtil.getName("", item.getSenduser()
						.getNickname(), item.getSenduser().getName()));
			}
			...
			if (!TextUtils.isEmpty(item.getReplyuser().getUserid())) {
				span.append("回复");
				if (userId.equals(item.getReplyuser().getUserid())) {
					span.append("我");
					span.setSpan(new ForegroundColorSpan(mContext
						...
						.getReplyuser().getNickname(), item.getReplyuser()
						.getName()))) {
					span.append(SelectNameUtil.getName("", item.getReplyuser()
							.getNickname(), item.getReplyuser().getName()));
					span.setSpan(
							new ForegroundColorSpan(mContext.getResources()
									.getColor(R.color.blue)),
							span.length()
									- SelectNameUtil.getName("",
											item.getReplyuser().getNickname(),
											item.getReplyuser().getName())
											.length(), span.length(),
							Spannable.SPAN_INCLUSIVE_EXCLUSIVE);
				}
			}
