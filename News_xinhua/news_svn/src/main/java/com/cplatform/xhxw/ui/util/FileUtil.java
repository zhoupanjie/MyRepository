...
public class FileUtil {
     ...
     * @param dstOs dstFile
     */
    public static boolean copyStream(FileInputStream srcIs, FileOutputStream dstOs) {
        try {

            byte[] buffer = new byte[8 * 1024];
            int length = 0;
            while ((length = srcIs.read(buffer)) != -1) {
                dstOs.write(buffer, 0, length);
            }
     ...
     * @param dstOs dstFile
     */
    public static boolean copyStream(InputStream srcIs, OutputStream dstOs) {
        try {
            byte[] buffer = new byte[8 * 1024];
            int length = 0;
            while ((length = srcIs.read(buffer)) != -1) {
    ...
    public static String file2Base64(String fileName) throws IOException {
        File file = new File(fileName);
        FileInputStream in = new FileInputStream(file);
        byte[] buffer = new byte[(int) file.length() + 100];
        int length = in.read(buffer);
        return Base64.encodeToString(buffer, 0, length,
                Base64.DEFAULT);
    }
}
