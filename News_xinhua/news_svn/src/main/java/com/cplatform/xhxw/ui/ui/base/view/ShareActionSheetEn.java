 ...
 * 
 */
public class ShareActionSheetEn extends PopupWindow {

	private static final String TAG = ShareActionSheetEn.class.getSimpleName();
	...
	private ShareActionSheetEn(View contentView, int width, int height,
			boolean focusable) {
		super(contentView, width, height, focusable);
	}

	public static ShareActionSheetEn getInstance(final Activity context,
		...
		LinearLayout contentView = (LinearLayout) LayoutInflater.from(context)
				.inflate(R.layout.action_sheet_en, null);
		more_lis = more;
		...
		final ShareActionSheetEn actionSheet = new ShareActionSheetEn(
		...
		int layout_id = 0;
		// if (isShowCollect) {
		// layout_id = R.layout.action_share_has_collect_en;
		// } else {
		layout_id = R.layout.action_share_no_collect_en;
		// }
		...
		/** 分享 */
		actionSheet.mController = UMServiceFactory
		...
		contentView.findViewById(R.id.share_qq).setOnClickListener(
				new OnClickListener() {
					...
					public void onClick(View v) {
						UmsShare(context, "QQ");
//						mController.getConfig().supportQQPlatform(context,
//								clickUrl);
				...
				new OnClickListener() {
					...
					public void onClick(View v) {
						...
						Toast.makeText(context.getApplicationContext(),
								"The link has been copied", Toast.LENGTH_SHORT)
								.show();
						actionSheet.dismiss();
					}
				});
	...
	private static void initMoreFuncPopup(View contentView) {
		...
		if (isShowCollect) {
			...
			// collect_tv = (TextView)
			// collect_layout.findViewById(R.id.textView1);
	...
	private static void initDisplayModel() {
		...
		if (model == Constants.DISPLAY_MODEL_DAY) {
			...
			detail_more_fun_night_model_tv.setText(R.string.day_model_en);
		} else {
			mDisplayModelIv.setBackgroundResource(R.drawable.detail_night);
			detail_more_fun_night_model_tv.setText(R.string.night_model_en);
		}
	}
	...
	// collect_tv.setText("Cancel");
	// } else {
	// collect_image.setImageResource(R.drawable.icon_star);
	// collect_tv.setText("Favorites");
	// }
	// }
	// }
	...
	private static boolean checkNetWork(Activity context,
			ShareActionSheetEn actionSheet) {
