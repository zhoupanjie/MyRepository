...
public class JsonHttpResponseHandler extends AsyncHttpResponseHandler {
		...
		if (statusCode != HttpStatus.SC_NO_CONTENT) {
			try {
				...
				if (jsonResponse instanceof JSONObject
						|| jsonResponse instanceof JSONArray) {
					sendMessage(obtainMessage(SUCCESS_JSON_MESSAGE,
							new Object[] { statusCode, jsonResponse }));
				} else {
					sendFailureMessage(new JSONException(
							"can not convert to json"), responseBody);
				}
			} catch (JSONException e) {
				sendFailureMessage(e, responseBody);
			}
		} else {
			sendFailureMessage(new JSONException("no json content"),
					responseBody);
		}
	}
	...
	protected void handleSuccessJsonMessage(int statusCode, Object jsonResponse) {
		AsyncHttpUtil.log(TAG, "handleSuccessJsonMessage:" + jsonResponse);
		if (jsonResponse instanceof JSONObject) {
	...
	protected void handleFailureMessage(Throwable e, String responseBody) {
		AsyncHttpUtil.log(TAG, "handleFailureMessage:" + responseBody);
		JSONObject jsonError = new JSONObject();
		try {
			jsonError.put("code", "1");
			jsonError.put("message", "网络连接失败");
			if (e instanceof UnknownHostException
					|| e instanceof SocketException
					|| e instanceof SocketTimeoutException
					|| e instanceof IOException
					|| e instanceof NullPointerException) {
				onFailure(e, jsonError);
